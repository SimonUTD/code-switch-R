...
## 🔄 MCP 外部变更同步 / Skills & Prompts UI 修复 / 监控与看板稳定性

- **MCP 外部变更自动对齐**：切换到 MCP 页时自动扫描 Claude/Codex/Gemini 的配置文件；外部新增会自动导入；外部删除会自动关闭本程序中对应平台开关（避免下次保存又写回去）
- **Skills 页面重构**：将“已安装（项目/用户）”与“未安装（目录/推荐）”分组展示，并补齐 Repo 管理与安装弹窗，逻辑更清晰
- **提示词页面修复**：顶部 Claude/Codex/Gemini 切换 Tab 交互与布局修复；编辑弹窗输入聚焦更稳定
- **可用性监控修复**：时间线（历史）列不再被挤压成“竖排”，表格布局与配置弹窗体验优化
- **更新状态修复**：启动加载 update-state 时会校正低于当前安装版本的 `latestKnownVersion`（避免出现“重启又显示 v1.3.4”）
- **日志关闭时的看板/请求清单**：日志开关关闭时热力图与请求清单不再误刷新/统计失败，并提供明确引导
...
## 🧩 MCP 配置增强 / 低功耗日志开关 / 表格化 UI

- **MCP 导入/同步**：支持 HTTP MCP `headers` 与 `startup_timeout_sec`（兼容多种字段别名），并同步写入 Codex 配置
...
- **看板热力图**：宽屏自动填充可用宽度，小屏仍支持横向滚动
- **看板折线图**：修复 token/金额长期为 0 导致“没数据”的问题，并新增“请求次数”折线
- **日志页面**：程序日志 / MITM 转发日志空状态与时间戳提升对比度，可读性更好
